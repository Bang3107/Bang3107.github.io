...
(function ($) {
  ...
  jQuery(document).ready(function ($) {
    ...
    $(".navbar-nav").on("click", "a", function (e) {
      ...
      if (href && href.startsWith("#") && $(href).length) {
        ...
        $("html, body")
          ...
          .animate(
            {
              scrollTop: $(href).offset().top - $(".navbar-fixed-top").outerHeight(),
            },
            800
          );
      }
    ...
    $(".project-hover").magnificPopup({ delegate: "a", type: "image" });
              ...
              if (!res.error) {
                ...
                $(".wish-box").prepend(
                  '<div class="wish-box-item bg"><strong>' +
                    $(form).find("input[name='name']").val().replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;") +
                    "</strong><p>" +
                    $(form).find("textarea[name='content']").val().replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;") +
                    "</p></div>"
                );
  ...
  function setAutoHeightEventImage() {
    if ($(".wedding-event").length) {
      if ($(window).width() >= 992) {
        ...
        $(".wedding-event .banner-event")
          .parent()
          .css("min-height", imageHeight + 70 + "px");
      } else {
